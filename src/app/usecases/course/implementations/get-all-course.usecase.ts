...
export class GetAllCourseUseCase implements IGetAllCourseUseCase {
  ...
  async execute(query: QueryCourse, authData: Payload): Promise<ResponseDTO> {
    try {
      ...
      if (role === "admin") {
        courses = await this._courseRepository.findAll(query);
      } else if (role === "mentor") {
        courses = await this._courseRepository.findAllByMentorId(query, userId);
      } else {
        courses = await this._courseRepository.findAllPublished(query);
      }
